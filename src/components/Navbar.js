...
export function renderNavbar() {
  ...
  const isAuthenticated = store.isAuthenticated && user;
  const view = store.currentView;
  const isAdmin = isAuthenticated && user.rol_global === 'ADMIN';
  const isMenuOpen = store.isProfileMenuOpen;
  const misLicenciasCount = isAuthenticated ? store.licencias.filter(l => l.id_usuario === user.id_usuario).length : 0;
  ...
  return `
    <nav class="navbar">
      <div class="container navbar-container">
        ...
        ` : `
          <!-- Botón Iniciar Sesión cuando no hay usuario autenticado -->
          <div>
            <button class="btn btn-primary btn-sm" onclick="window.navigate('login')">
